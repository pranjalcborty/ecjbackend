...
public class Evolve {
    ...
    public static EvolutionState initialize(ParameterDatabase parameters, int randomSeedOffset, Output output) {
        ...
        breedthreads = ec.Evolve.determineThreads(output, parameters, new Parameter(P_BREEDTHREADS));
        evalthreads = ec.Evolve.determineThreads(output, parameters, new Parameter(P_EVALTHREADS));
        ...
        for (x = 0; x < random.length; x++) {
            ...
            random[x] = ec.Evolve.primeGenerator(new MersenneTwisterFast(seeds[x]));    // we prime the generator to be more sure of randomness.
